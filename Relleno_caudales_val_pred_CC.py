...
#%%    
    ...
    ruta_Q_0_dga=[x.__str__() for x in list_arc_est if 'Atacama' in x.__str__()][0]
    ruta_Q_1_dga=[x.__str__() for x in list_arc_est if 'Coquimbo' in x.__str__()][0]
    ruta_Q_2_dga=[x.__str__() for x in list_arc_est if 'Valpara' in x.__str__()][0]
    ...
    # sacar desague de vertedero
    q_day=q_day[[x for x in q_day.columns if '08133013-1' not in x ]]
...
if __name__=='__main__':
    ...
    #%%                    ----------------------------LIMARI-------------------------------
        ...
        # rutas
        root='.'
        ruta_Q_0_dga=join_path(root,'..','Scripts','inputs','CaudalesDGA_Coquimbo_diarios_2021.xlsx')
        ...
        # leer caudales
        # dga
        q_day=pd.read_excel(ruta_Q_0_dga,sheet_name='Datos',index_col=0,
                                  parse_dates=True,skiprows=[1])

        #filtrar estaciones que sean canales, vertederos, desagues
        names_blacklist=['canal','desague','vertedero','dren']
        
        # leer metadata dga de Coquimbo
        metadata=pd.read_excel(ruta_Q_0_dga,sheet_name='Ficha_est',index_col=0)
        # metadata_0_dga[['Lon','Lat']]=metadata_0_dga[['Lon','Lat']].applymap(lambda x:parse_dms(x))
        ...
        # promediar caudales medios diarios considerando flags y meses con mas
        # de 20 registros
        flags = df = pd.DataFrame(columns = q_day.columns, 
                   index = q_day.index)
        q_mon=day_to_mon(q_day,flags)
        ...
        while df_check.isnull().values.any():
            for col in estaciones:
                print('Rellenando estación '+str(col),
                      metadata[metadata['rut']==col]['Estacion'].iloc[0])
    ...
    # Rellenando estación 04531001-9 Rio cogoti en cogoti 18 DGA
    # ['Rio cogoti en cogoti 18 DGA' 'Rio cogoti en fraguita DGA'
    #  'Rio cogoti entrada embalse cogoti DGA'
    #  'Rio pama entrada embalse cogoti DGA']
    # ['Rio cogoti en cogoti 18 DGA' 'Rio cogoti en fraguita DGA'
    #  'Rio cogoti entrada embalse cogoti DGA'
    #  'Rio combarbala en ramadillas DGA']
    # ['Rio cogoti en cogoti 18 DGA' 'Rio cogoti en fraguita DGA'
    #  'Rio cogoti entrada embalse cogoti DGA']
    # ['Rio cogoti en cogoti 18 DGA' 'Rio combarbala en ramadillas DGA'
    #  'Rio mostazal antes junta rio tulahuencito DGA'
    #  'Rio pama entrada embalse cogoti DGA']
    # ['Rio cogoti en cogoti 18 DGA' 'Rio cogoti entrada embalse cogoti DGA'
    #  'Rio grande en puntilla san juan DGA'
    #  'Rio pama entrada embalse cogoti DGA']
    # ['Rio cogoti en cogoti 18 DGA' 'Rio cogoti en fraguita DGA'
    #  'Rio cogoti entrada embalse cogoti DGA'
    #  'Rio combarbala en ramadillas DGA']
    # ['Rio cogoti en cogoti 18 DGA' 'Rio combarbala en ramadillas DGA'
    #  'Rio mostazal antes junta rio tulahuencito DGA'
    #  'Rio mostazal en caren DGA']
    # ['Rio cogoti en cogoti 18 DGA' 'Rio combarbala en ramadillas DGA'
    #  'Rio mostazal antes junta rio tulahuencito DGA'
    #  'Rio mostazal en cuestecita DGA']
    # ['Rio cogoti en cogoti 18 DGA' 'Rio combarbala en ramadillas DGA'
    #  'Rio grande en las ramadas DGA' 'Rio rapel en paloma DGA']
    # ['Rio cogoti en cogoti 18 DGA' 'Rio cogoti en fraguita DGA'
    #  'Rio cogoti entrada embalse cogoti DGA'
    #  'Rio combarbala en ramadillas DGA']
    # ['Rio cogoti en cogoti 18 DGA' 'Rio cogoti en fraguita DGA'
    #  'Rio grande en las ramadas DGA' 'Rio tascadero en desembocadura DGA']
    # ['Rio cogoti en cogoti 18 DGA' 'Rio grande en coipo DGA'
    #  'Rio grande en las ramadas DGA' 'Rio pama entrada embalse cogoti DGA']
    #%% plots para presentación
